...
bool trace(int y, int x) {
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    if (is_out(ny, nx) || park_dist[ny][nx] == -1) 
        continue;
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    while(q.size())
    {
      ...
      for (int dir : look_dir) {
        ...
        if (is_out(ny, nx) || visited[i][ny][nx]) 
            continue;
        ...
        if (warrior_land[ny][nx]) {
          ...
          {
            // 기울기 양수
            if ( (double)(ny - my) / (double)(nx - mx) > 0) // 소숫값 유의
            {
...
void warrior_move(int flag)
{
  ...
  for (Warrior& w : warrior_vec)
  {
    ...
    for (int i = 0; i < 4; i++)
    {
      ...
      if (is_out(ny, nx) || visited[max_freeze_dir][ny][nx] == 1) 
        continue;
