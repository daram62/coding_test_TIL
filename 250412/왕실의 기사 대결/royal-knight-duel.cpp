...
struct Node { int y, x; }; // 위치 정보용 구조체
...
bool check(int curNum, int dir) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (i == dir) { // 이동 방향인 경우
                if (ny < 0 || nx < 0 || ny >= L || nx >= L || MAP[ny][nx] == 2) return false;
                if (visited[ny][nx] || knightMAP[ny][nx] == 0) continue;
            } else { // 상우하좌 중 이동방향 아닌 곳: 같은 기사 이어붙이기
                if (ny < 0 || nx < 0 || ny >= L || nx >= L) continue;
                if (visited[ny][nx] || knightMAP[ny][nx] == 0 ||
                    knightMAP[now.y][now.x] != knightMAP[ny][nx]) continue;
            }
...
void push(int curNum, int dir) {
    if (!check(curNum, dir)) return;

    // 밀릴 기사들의 기존 위치 지우기
    for (int i = 1; i <= N ; i++) {
        if (!pushList[i]) continue;
...
void solve() {
    for (int i = 0; i < query.size(); i++) {
        ...
        int dir = query[i].x;
        if (isDead[curNum]) continue; // 죽은 기사 무시
        push(curNum, dir);
    }
}
